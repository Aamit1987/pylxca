...
class lxca_api(with_metaclass(Singleton, object)):
    ...
    def api(self, object_name, dict_handler=None, con=None):
        ...
        try:
            ...
            if con is None and self.con and isinstance(
                    self.con, lxca_connection):
                con = self.con
            ...
            logger.error(
                "Exception %s Occurred while calling REST API for object %s" %
                (re, object_name))
            raise re
        except ValueError as re:
            logger.error(
                "Exception ValueError %s Occurred while calling REST API for object %s" %
                (re, object_name))
            raise re
        except AttributeError as re:
            logger.error(
                "Exception AttributeError %s Occurred while calling REST API for object %s" %
                (re, object_name))
            raise re
    ...
    def connect(self, dict_handler=None):
        ...
        if dict_handler:
            url = next(
                item for item in [
                    dict_handler.get('l'),
                    dict_handler.get('url')] if item is not None)
            user = next(
                item for item in [
                    dict_handler.get('u'),
                    dict_handler.get('user')] if item is not None)
            passwd = next(
                item for item in [
                    dict_handler.get('p'),
                    dict_handler.get('pw')] if item is not None)
    ...
    def set_log_level(self, log_value):
        try:
            ...
            logger.debug("Current Log Level is now set to " +
    ...
    def get_chassis(self, dict_handler=None):
        ...
        if dict_handler:
            uuid = next(
                (item for item in [
                    dict_handler.get('u'),
                    dict_handler.get('uuid')] if item is not None),
                None)
            status = next(
                (item for item in [
                    dict_handler.get('s'),
                    dict_handler.get('status')] if item is not None),
                None)

        resp = lxca_rest().get_chassis(
            self.con.get_url(), self.con.get_session(), uuid, status)
    ...
    def get_nodes(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            chassis_uuid = next(
                (item for item in [
                    dict_handler.get('c'),
                    dict_handler.get('chassis')] if item is not None),
                None)

        if chassis_uuid:
            resp = lxca_rest().get_chassis(
                self.con.get_url(),
                self.con.get_session(),
                chassis_uuid,
                status)
            py_obj = json.loads(resp.text)
            py_obj = {'nodesList': py_obj["nodes"]}
        else:
            resp = lxca_rest().get_nodes(
                self.con.get_url(), self.con.get_session(), uuid, status)
            py_obj = json.loads(resp.text)
        return py_obj
    ...
    def get_switches(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            port_name = next(
                (item for item in [
                    dict_handler.get('ports')] if item is not None),
                None)
            action = next(
                (item for item in [
                    dict_handler.get('action')] if item is not None),
                None)
            if "ports" in dict_handler:
                list_port = True

        if chassis_uuid:
            resp = lxca_rest().get_chassis(
                self.con.get_url(), self.con.get_session(), chassis_uuid, None)
            py_obj = json.loads(resp.text)
            py_obj = {'switchesList': py_obj["switches"]}
        elif list_port and (action is None):
            resp = lxca_rest().get_switches_port(
                self.con.get_url(), self.con.get_session(), uuid, list_port)
            py_obj = json.loads(resp.text)
        elif port_name and action:
            resp = lxca_rest().put_switches_port(
                self.con.get_url(), self.con.get_session(), uuid, port_name, action)
            py_obj = json.loads(resp.text)
        else:
    ...
    def get_powersupply(self, dict_handler=None):
        ...
        if chassis_uuid:
            ...
            resp = lxca_rest().get_powersupply(
                self.con.get_url(), self.con.get_session(), uuid)
            py_obj = json.loads(resp.text)
        return py_obj
    ...
    def get_scalablesystem(self, dict_handler=None):
        ...
        if dict_handler:
            complexid = next(
                (item for item in [
                    dict_handler.get('i'),
                    dict_handler.get('id')] if item is not None),
                None)
            complextype = next(
                (item for item in [
                    dict_handler.get('t'),
                    dict_handler.get('type')] if item is not None),
                None)

        resp = lxca_rest().get_scalablesystem(
            self.con.get_url(), self.con.get_session(), complexid, complextype)
        py_obj = json.loads(resp.text)
        return py_obj
    ...
    def do_discovery(self, dict_handler=None):
        ...
        if dict_handler:
            ip_addr = next(
                (item for item in [
                    dict_handler.get('i'),
                    dict_handler.get('ip')] if item is not None),
                None)
            jobid = next(
                (item for item in [
                    dict_handler.get('j'),
                    dict_handler.get('job')] if item is not None),
                None)

        resp = lxca_rest().do_discovery(
            self.con.get_url(), self.con.get_session(), ip_addr, jobid)
    ...
    def do_manage(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            rpw = next(
                (item for item in [
                    dict_handler.get('r'),
                    dict_handler.get('rpw')] if item is not None),
                None)
            jobid = next(
                (item for item in [
                    dict_handler.get('j'),
            ...
            force = next(
                (item for item in [
                    dict_handler.get('f'),
                    dict_handler.get('force')] if item is not None),
                None)

        resp = lxca_rest().do_manage(
            self.con.get_url(),
            self.con.get_session(),
            ip_addr,
            user,
    ...
    def get_configtargets(self, dict_handler=None):
        ...
        resp = lxca_rest().get_configtargets(
            self.con.get_url(), self.con.get_session(), targetid)
    ...
    def do_configpatterns(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            patternname = next(
                (item for item in [
                    dict_handler.get('n'),
                    dict_handler.get('name')] if item is not None),
                None)
            includeSettings = next(
                (item for item in [
                    dict_handler.get('includeSettings')] if item is not None),
                None)
            endpoint = next(
                (item for item in [
                    dict_handler.get('e'),
                    dict_handler.get('endpoint')] if item is not None),
                None)
            restart = next(
                (item for item in [
                    dict_handler.get('r'),
                    dict_handler.get('restart')] if item is not None),
                None)
            etype = next(
                (item for item in [
                    dict_handler.get('t'),
                    dict_handler.get('type')] if item is not None),
                None)
            pattern_update_dict = next(
                (item for item in [
                    dict_handler.get('pattern_update_dict')] if item is not None),
                None)
            status = next(
                (item for item in [
                    dict_handler.get('s'),
        ...
        if patternname and not patternid:
            ...
            resp = lxca_rest().do_configpatterns(
                self.con.get_url(),
                self.con.get_session(),
                None,
                None,
        ...
        if status:
            resp = lxca_rest().get_configstatus(
                self.con.get_url(), self.con.get_session(), endpoint)
        else:
            resp = lxca_rest().do_configpatterns(
                self.con.get_url(),
                self.con.get_session(),
    ...
    def get_configprofiles(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            delete = next(
                (item for item in [
                    dict_handler.get('d'),
                    dict_handler.get('delete')] if item is not None),
                None)
            unassign = next(
                (item for item in [
                    dict_handler.get('u'),
                    dict_handler.get('unassign')] if item is not None),
                None)
            powerdown = next(
                (item for item in [
                    dict_handler.get('p'),
                    dict_handler.get('powerdown')] if item is not None),
                None)
            resetimm = next(
                (item for item in [
                    dict_handler.get('resetimm')] if item is not None),
                None)
            force = next(
                (item for item in [
                    dict_handler.get('f'),
        ...
        if profilename:
            resp = lxca_rest().put_configprofiles(
                self.con.get_url(), self.con.get_session(), profileid, profilename)
        elif endpoint and restart:
            resp = lxca_rest().post_configprofiles(
                self.con.get_url(), self.con.get_session(), profileid, endpoint, restart)
        elif profileid and delete and delete.lower() == 'true':
            resp = lxca_rest().delete_configprofiles(
                self.con.get_url(), self.con.get_session(), profileid)
        elif profileid and unassign and unassign.lower() == 'true':
            resp = lxca_rest().unassign_configprofiles(
                self.con.get_url(),
                self.con.get_session(),
                profileid,
                powerdown,
            ...
            resp = lxca_rest().get_configprofiles(
                self.con.get_url(), self.con.get_session(), profileid)
    ...
    def get_jobs(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            state = next(
                (item for item in [
                    dict_handler.get('s'),
                    dict_handler.get('state')] if item is not None),
                None)
            canceljobid = next(
                (item for item in [
                    dict_handler.get('c'),
                    dict_handler.get('cancel')] if item is not None),
                None)
            deletejobid = next(
                (item for item in [
                    dict_handler.get('d'),
    ...
    def get_lxcalog(self, dict_handler=None):
        ...
        if dict_handler:
            filter = next(
                (item for item in [
                    dict_handler.get('f'),
                    dict_handler.get('filter')] if item is not None),
                None)
    ...
    def do_updatepolicy(self, dict_handler=None):
        ...
        if dict_handler:
            policy = next(
                (item for item in [
                    dict_handler.get('p'),
                    dict_handler.get('policy')] if item is not None),
                None)
            info = next(
                (item for item in [
                    dict_handler.get('i'),
                    dict_handler.get('info')] if item is not None),
                None)
            uuid = next(
                (item for item in [
                    dict_handler.get('u'),
            ...
            jobid = next(
                (item for item in [
                    dict_handler.get('j'),
                    dict_handler.get('jobid')] if item is not None),
                None)
            type = next(
                (item for item in [
                    dict_handler.get('t'),
        ...
        if policy:
            resp = lxca_rest().post_updatepolicy(
                self.con.get_url(), self.con.get_session(), policy, type, uuid)
        else:
            resp = lxca_rest().get_updatepolicy(
                self.con.get_url(), self.con.get_session(), info, jobid)
    ...
    def get_updaterepo(self, dict_handler=None):
        ...
        if dict_handler:
            key = next(
                (item for item in [
                    dict_handler.get('k'),
                    dict_handler.get('key')] if item is not None),
                None)
            action = next(
                (item for item in [
                    dict_handler.get('a'),
            ...
            mt = next(
                (item for item in [
                    dict_handler.get('m'),
                    dict_handler.get('mt')] if item is not None),
                None)
            scope = next(
                (item for item in [
                    dict_handler.get('s'),
                    dict_handler.get('scope')] if item is not None),
                None)
            fixids = next(
                (item for item in [
                    dict_handler.get('f'),
                    dict_handler.get('fixids')] if item is not None),
                None)
            type = next(
                (item for item in [
                    dict_handler.get('t'),
        ...
        if key:
            resp = lxca_rest().get_updaterepo(
                self.con.get_url(), self.con.get_session(), key, mt, scope)
        elif action:
            resp = lxca_rest().put_updaterepo(
                self.con.get_url(),
                self.con.get_session(),
                action,
                fixids,
    ...
    def do_managementserver(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            files = next(
                (item for item in [
                    dict_handler.get('files')] if item is not None),
                None)
        if key:
            resp = lxca_rest().get_managementserver(
                self.con.get_url(), self.con.get_session(), key, fixids, type)
        elif action:
            resp = lxca_rest().set_managementserver(
                self.con.get_url(), self.con.get_session(), action, files, jobid, fixids)
        else:
            resp = lxca_rest().get_managementserver(
                self.con.get_url(), self.con.get_session(), key, fixids, type)
    ...
    def do_updatecomp(self, dict_handler=None):
        ...
        if dict_handler:
            mode = next(
                (item for item in [
                    dict_handler.get('m'),
                    dict_handler.get('mode')] if item is not None),
                None)
            action = next(
                (item for item in [
                    dict_handler.get('a'),
            ...
            server = next(
                (item for item in [
                    dict_handler.get('s'),
                    dict_handler.get('server')] if item is not None),
                None)
            storage = next(
                (item for item in [
                    dict_handler.get('t'),
                    dict_handler.get('storage')] if item is not None),
                None)
            switch = next(
                (item for item in [
                    dict_handler.get('w'),
                    dict_handler.get('switch')] if item is not None),
                None)
            cmm = next(
                (item for item in [
                    dict_handler.get('c'),
                    dict_handler.get('cmm')] if item is not None),
                None)

        resp = lxca_rest().do_updatecomp(
            self.con.get_url(),
            self.con.get_session(),
            mode,
            action,
    ...
    def get_set_tasks(self, dict_handler=None):
        ...
        if dict_handler:
            job_uuid = next(
                (item for item in [
                    dict_handler.get('jobUID')] if item is not None),
                None)
            includeChildren = next(
                (item for item in [
                    dict_handler.get('children')] if item is not None),
                "false")
            ...
            updateList = next(
                (item for item in [
                    dict_handler.get('updateList')] if item is not None),
                None)
            #state = next((item for item in [dict_handler.get('state')] if item is not None), None)

        if job_uuid and action in ['cancel', 'delete']:
            resp = lxca_rest().put_tasks(
                self.con.get_url(), self.con.get_session(), job_uuid, action)
            py_obj = resp.status_code
        elif action in ['update']:
            resp = lxca_rest().put_tasks_update(
                self.con.get_url(), self.con.get_session(), updateList)
            py_obj = resp.status_code
        elif job_uuid:
            resp = lxca_rest().get_tasks_list(
                self.con.get_url(),
                self.con.get_session(),
                job_uuid,
                includeChildren)
    ...
    def get_set_manifests(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            filepath = next(
                (item for item in [
                    dict_handler.get('f'),
                    dict_handler.get('file')] if item is not None),
                None)

        resp = lxca_rest().get_set_manifests(
            self.con.get_url(), self.con.get_session(), sol_id, filepath)
    ...
    def get_set_osimage(self, dict_handler=None):
        ...
        if get_method:
            ...
            resp = lxca_rest().get_osimage(
                osimages_info,
                url=self.con.get_url(),
                session=self.con.get_session(),
                **kwargs)
        else:
            resp = lxca_rest().set_osimage(
                osimages_info,
                url=self.con.get_url(),
                session=self.con.get_session(),
                **kwargs)
    ...
    def get_set_resourcegroups(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            desc = next(
                (item for item in [
                    dict_handler.get('d'),
                    dict_handler.get('description')] if item is not None),
                None)
            type = next(
                (item for item in [
                    dict_handler.get('t'),
            ...
            solutionVPD = next(
                (item for item in [
                    dict_handler.get('s'),
                    dict_handler.get('solutionVPD')] if item is not None),
                None)
            members = next(
                (item for item in [
                    dict_handler.get('m'),
                    dict_handler.get('members')] if item is not None),
                None)
            criteria = next(
                (item for item in [
                    dict_handler.get('c'),
                    dict_handler.get('criteria')] if item is not None),
                None)

        resp = lxca_rest().get_set_resourcegroups(
            self.con.get_url(),
            self.con.get_session(),
            uuid,
            name,
        ...
        if dict_handler:
            ...
            name = next(
                (item for item in [
                    ...
                    dict_handler.get('name')] if item is not None),
                None)
            targetResourceType = next(
                (item for item in [
                    dict_handler.get('t'),
                    dict_handler.get('targetResourceType')] if item is not None),
                None)
            targetGroup = next(
                (item for item in [
                    dict_handler.get('g'),
                    dict_handler.get('targetGroup')] if item is not None),
                None)
            content = next(
                (item for item in [
                    dict_handler.get('c'),
                    dict_handler.get('content')] if item is not None),
                None)

        if name:
            resp = lxca_rest().set_rules(
                self.con.get_url(),
                self.con.get_session(),
                name,
                targetResourceType,
    ...
    def get_set_compositeResults(self, dict_handler=None):
        ...
        if dict_handler:
            ...
            solutionGroup = next(
                (item for item in [
                    dict_handler.get('s'),
                    dict_handler.get('solutionGroup')] if item is not None),
                None)

        if solutionGroup:
            resp = lxca_rest().set_compositeResults(
                self.con.get_url(), self.con.get_session(), solutionGroup)
        else:
            resp = lxca_rest().get_compositeResults(
                self.con.get_url(), self.con.get_session(), id)
        py_obj = json.loads(resp.text)
        return py_obj
