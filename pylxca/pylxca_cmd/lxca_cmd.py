...
class connect(InteractiveCommand):
    ...
    def handle_command(self, opts, args):
        ...
        try:
            opts, argv = getopt.getopt(
                args, self.get_char_options(), self.get_long_options())
        ...
        try:
            ...
            self.sprint(
                "Exception %s occurred while executing command." %
                (err))
    ...
    def handle_no_input(self):
        """
        @summary:
        Use this function without any external input

        """
        # no_opt action can differ command to command so override this function
        # if required
        self.invalid_input_err()
        return
    ...
    def handle_output(self, out_obj):
        """
        @summary:
        Use this function with output

        @param
        The parameters for this command are as follows

        con_obj        Connection Object to Lenovo XClarity Administrator
        """
        if out_obj is None:
...
class log(InteractiveCommand):
    ...
    def handle_no_input(self, con_obj=None):
        ...
        """
        api = pylxca.pylxca_api.lxca_api()
        self.sprint("Current Log Level is set to " +
                    str(logging.getLevelName(api.get_log_level())))
        message = """
...
class ostream(InteractiveCommand):
    ...
    def handle_no_input(self):
        ...
        """
        self.sprint(
            "Current ostream level is set to %s" %
            (self.shell.ostream.get_lvl()))
        message = """
Possible ostream levels, Please use following values to set desired stdout
level.
...
"""
    ...
    def handle_output(self, out_obj):
        ...
        if out_obj:
            ...
            message = """
Possible ostream levels, Please use following values to set desired ostream
level.
...
class nodes(InteractiveCommand):
    ...
    USAGE:
        nodes -h
        nodes [-u <node UUID>] [-s <managed/unmanaged>] [-c <chassis UUID>]
              [-v <view filter name>]
...
class switches(InteractiveCommand):
    ...
    def handle_command(self, opts, args):
        ...
        try:
            ...
            change = False
        if change:
            args = [w.replace('--ports', '--ports=') if w ==
                    "--ports" else w for w in args]
...
class powersupplies(InteractiveCommand):
    ...
    USAGE:
        powersupplies -h
        powersupplies [-u <power supply UUID>] [-c <chassis UUID>]
                      [-v <view filter name>]
...
class scalablesystem(InteractiveCommand):
    ...
    USAGE:
        scalablesystem -h
        scalablesystem [-i <scalablesystem id>] [-t <scalablesystem type>]
                       [-s <status>] [-v <view filter name>]
...
class discover(InteractiveCommand):
    ...
    OPTIONS:
        -i, --ip       One or more IP addresses for each endpoint to
                       be discovered.
...
class manage(InteractiveCommand):
    ...
    USAGE:
        ...
        manage  -i <IP Address of endpoint> -u <user ID to access the endpoint>
                -p <current password to access the endpoint>
                [-r <recovery password for the endpoint>]
                [-f <Force Manage (True/False)>]
        manage  -j <job ID> [-v <view filter name>]

    OPTIONS:
        -i, --ip        One or more IP addresses for each endpoint
                        to be managed.
...
class unmanage(InteractiveCommand):
    ...
    OPTIONS:
        -e, --ep    one or more endpoints to be unmanaged.
                This is comma separated list of multiple endpoints,
                each endpoint should
                contain endpoint information separated by semicolon.
                endpoint's IP Address
                (multiple addresses should be separated by #),
                UUID of the endpoint and
                Type of endpoint to be unmanaged ,
                This can be one of the following values:
                    ...
                    Rackswitch
                    Rack-Tower
        -f, --force     Indicates whether to force the unmanagement of
                        an endpoint (True/False)
...
class users(InteractiveCommand):
    ...
    USAGE:
        users [-i <unique ID of the user to be retrieved>]
              [-v <view filter name>]
...
class updatepolicy(InteractiveCommand):
    ...
    USAGE:
        ...
        updatepolicy -p <Compliance policy to be assigned to device>
         -u <UUID of Device> -t <Device Type>
    ...
    OPTIONS:
        ...
        -u, --UUID      UUID of the device to which you want to
                        assign the compliance policy
        -t, --type      Type = The device type. This can be one of the
                        following values.
        ...
        -i, --info      Specifies the type of information to return.
                             This can be one of the following values:
                            FIRMWARE- Returns information about firmware that
                            is applicable to each managed endpoint
                            RESULTS- Returns persisted compare result for
                            servers to which a compliance policy is assigned
...
class updaterepo(InteractiveCommand):
    ...
    USAGE:
        updaterepo -k <Key to return the specified type of update>
                    [-v <view filter name>]
        updaterepo -a <action to take> [-v <view filter name>]
    OPTIONS:
        -k, --key       Returns the specified type of update.
                        This can be one of the following values.
                        supportedMts - Returns a list of supported machine types
                        size - Returns the repository size
                        lastRefreshed - Returns the timestamp of the
                        last repository refresh
                        importDir - Returns the import directory for the
                        repository.
                        publicKeys - Returns the supported signed keys
                        updates - Returns information about all firmware updates
                        updatesByMt - Returns information about firmware
                        updates for the specified machine type
                        updatesByMtByComp - Returns the update component
                        names for the specified machine type
        -a  --action    The action to take. This can be one of the
                        following values.
                        read - Reloads the repository files. The clears
                        the update information in cache and reads the update
                        file again from the repository.
                        refresh - Retrieves information about the latest
                        available firmware updates from the Lenovo Support
                        website, and stores the information to the
                        firmware-updates repository.
                        acquire - Downloads the specified firmware updates
                        from Lenovo Support website, and stores the updates
                        to the firmware-updates repository.
                        delete - Deletes the specified firmware updates
                        from the firmware-updates repository.
                        export.not supported

        -m  --mt        comma separated machine types
        -s  --scope     scope of operation [ all/latest] for refresh and
                        [ payloads ] for acquire
...
class updatecomp(InteractiveCommand):
    ...
    USAGE:
        ...
        updatecomp  [-m <activate mode>] [-a <action to take>]
        [-c <information of cmms>] [-w <information of switches>]
        [-s <information of servers>] [-t <information of storages>]
        updatecomp  -a power [-c <cmms UUID and desired state>]
         [-w <switches UUID and desired state>]
         [-s <servers UUID and desired state>]

    OPTIONS:
        -q, --query     The data to return. This can be one of the
                        following values.
                            components - Returns a list of endpoints and
                            components that can be updated.
                            status - Returns the status and progress
                            of firmware updates. This is the default value
        -m, --mode      Indicates when to activate the update.
                            This can be one of the following values.
                            immediate - Uses Immediate Activation mode
                            when applying firmware updates to the selected endpoints.
                            delayed - Uses Delayed Activation mode when
                            applying firmware updates to the selected endpoints.
        -a, --action    The action to take. This can be one of the following values.
                            apply - Applies the associated firmware to the
                            submitted components.
                            power - Perform power action on selected endpoint.
                            cancelApply - Cancels the firmware update
                            request to the selected components.
        -c, --cmm       cmms information
        -w, --switch    switch information
        -s, --server    servers information
        -t, --storage   storages information

            For action = apply/cancelApply, Device information should contain
            following data separated by comma
                UUID - UUID of the device
                Fixid - Firmware-update ID of the target package to be applied
                to the component. If not provided assigned policy would be used.
                Component - Component name

            For action = power, Device information should contain
            following data separated by comma
                UUID - UUID of the device
                powerState - One of the power state values.
                Possible values per device type are
                    Server: powerOn, powerOff, powerCycleSoft,
                    powerCycleSoftGraceful, powerOffHardGraceful
...
class configtargets(InteractiveCommand):
    ...
    USAGE:
        configtargets [-i <ID of specific profile or pattern>]
                      [-v <view filter name>]

    OPTIONS:
        -i, --id    The unique ID that was assigned when the server
                    pattern was created
        -v, --view    View filter name
...
class configpatterns(InteractiveCommand):
    ...
    USAGE:
        configpatterns [-i <ID of specific pattern>]
        [-r <when to activate the configurations>]
        [-e <Comma separated list of one or more UUIDs for the target servers>]
         [-t <type of the target server>] [-s True] [-v <view filter name>]

    OPTIONS:
        -i, --id    The unique ID that was assigned when the server
                    pattern was created
        -n, --name  Name of pattern
        -e, --endpoint    Comma separated list of one or more UUIDs for the
                          target servers,If a target is an empty bay,
                specify the location ID; otherwise, specify the server UUID
        -r, --restart    When to activate the configurations.
                This can be one of the following values:
                defer - Activate IMM settings but do not restart the server.
                immediate - Activate all settings and restart the
                            server immediately.
                pending - Manually activate the server profile and
                          restart the server.
...
class configprofiles(InteractiveCommand):
    ...
    USAGE:
        ...
        configprofiles -i <ID of specific profile> -e <UUID or location ID>
        -r <restart option> [-v <view filter name>]
        configprofiles -i <ID of specific profile>  -d <boolean>
        [-v <view filter name>]
        configprofiles -i <ID of specific profile>  -u <boolean> -p <boolean>
        --resetimm <boolean> -f <boolean> [-v <view filter name>]

    OPTIONS:
        -i, --id    The unique ID that was assigned when the server
                    pattern was created
              id    The unique ID that was assigned when the
                    server profile was created
        -n, --name        profile name
        -e,  --endpoint    endpoint  UUID of the server or location
                           id for flex system
        -r,  --restart     restart server to activate profile
                          ( immediate / defer )
...
class manifests(InteractiveCommand):
    """
    Send solution manifest to and retreive manifests from
