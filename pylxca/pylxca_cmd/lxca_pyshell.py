...
Girish Kumar <gkumar1@lenovo.com>
@license: Lenovo License
@copyright: Copyright 2016, Lenovo
@organization: Lenovo
@summary: This module provides scriptable interfaces and scriptable
python shell.
'''
...
@example
    con1 = connect( con = "https://10.243.12.142",user = "USERID",
                    pw = "Password", noverify = "True")
    '''
    global shell_obj
...
@param
    ...
    uuid          uuid of switch
    chassis       chassis uuid
    ports         empty ports string list all ports for uuid,
...
@param
    The parameters for this command are as follows
        ...
        Note : mp, type and epuuid parameters are dedpriciated and only
        kept for backword compatibility.

@example

        jobid = manage(con=con1,ip="10.243.6.68",user="USERID",
                       pw="PASSW0RD",rpw="PASSW0RD")

    or with named variable it can be represented as

        jobid = manage(con= con1,ip="10.243.6.68",user="USERID",
                       "PASSW0RD","PASSW0RD",True)
...
@param
    The parameters for this command are as follows
        ...
        ip          one or more endpoints to be unmanaged.
                    ...
                    each endpoint should contain endpoint information
                    separated by semicolon.
                    endpoint's IP Address(multiple addresses should be
                    separated by #), UUID of the endpoint and
                    ...
                    This can be one of the following values:
                          ...
                          Rackswitch
                          Rack-Tower
        force       Indicates whether to force the unmanagement of an
                    endpoint (True/False)
...
@param
    The parameters for this command are as follows
        ...
        endpoint    List of one or more UUIDs for the target servers,
                     If a target is an empty bay,
                      specify the location ID; otherwise, specify the server UUID

        restart     When to activate the configurations.
                     This can be one of the following values:
                      defer - Activate IMM settings but do not restart
                      the server.
                      immediate - Activate all settings and restart the
                      server immediately.
                      pending - Manually activate the server
...
@summary:
    ...
    Where KeyList is as follows
        ...
        keylist = ['con', 'id', 'name', 'endpoint', 'restart', 'delete',
        'unassign', 'powerdown', 'resetimm', 'force']

@param
    The parameters for this command are as follows

        id          The unique ID that was assigned when the server
                    profile was created
        name        profile name
...
@summary:
    Use this function to get config pattern targets
...
@summary:
    Use this function to read Firmwar update Policy
    from Lenovo XClarity Administrator
    run this function as
...
@param
    ...
    info    Specifies the type of information to return.
           This can be one of the following values:
                FIRMWARE- Returns information about firmware
                          that is applicable to each managed endpoint
...
@param
    ...
    key    Returns the specified type of update. This can be one of
    the following values.
                ...
                for the specified machine type
                updatesByMtByComp - Returns the update component names
                for the specified machine type

    action    The action to take. This can be one of the following values.
                read - Reloads the repository files. The clears the update
                information in cache and reads the update file again from
                the repository.
                refresh - Retrieves information about the latest available
                 firmware updates from the Lenovo Support website,
                         and stores the information to the firmware-updates
                          repository.
                acquire - Downloads the specified firmware updates from Lenovo
                 Support website, and stores the updates to the
                 firmware-updates repository.
                delete - Deletes the specified firmware updates from the
                firmware-updates repository.
...
@summary:
    Use this function to update firmware of endpoint from
    ...
    USAGE:
        ...
        keylist = ['con','query','mode','action','cmm','switch','server',
        'storage']
...
@param
    ...
    query   The data to return. This can be one of the following values.
                components - Returns a list of devices and components that
                can be updated.
                status - Returns the status and progress of firmware updates.
                This is the default value

    mode    Indicates when to activate the update. This can be one of the
    following values.
                immediate - Uses Immediate Activaton mode when applying
                firmware updates to the selected endpoints.
                delayed - Uses Delayed Activaton mode when applying firmware
                updates to the selected endpoints.

    action  The action to take. This can be one of the following values.
                apply - Applies the associated firmware to the submitted
                components.
                power - Perform power action on selected endpoint.
                cancelApply - Cancels the firmware update request to the
                selected components.
    ...
    storage storages information
            ...
            For action = apply/cancelApply, Device information should
            contain following data separated by comma
                UUID - UUID of the device
                Fixid - Firmware-update ID of the target package to be
                applied to the component. If not provided assigned policy
                would be used.
            ...
            following data separated by comma
                UUID - UUID of the device
                powerState - One of the power state values. Possible
                values per device type are
...
@summary:
    Use this function to send solution manifest to and retreive
    manifests from Lenovo XClarity Administrator
    run this function as
...
@summary:
    Use this function to Create, modify, delete or read resource group
    from Lenovo XClarity Administrator
    run this function as

    data_dictionary = resourcegroups( con_handle,uuid, name, desc, type,
    solutionVPD, members, criteria )

    Where KeyList is as follows

        keylist = ['uuid', 'name','description','type','solutionVPD',
        'members','criteria']
...
@param
    The parameters for this command are as follows
        ...
        type=         Type of Resource Group. <{"static", "dynamic",
        "solution"}>,
...
def _validate_param(
        ...
        **kwargs):
    '''
     this function will create param_dict and con from args and kwargs,
     param_dict will have only long options for keys,
     it will convert short option to long option key and
     finally validate parameters
    ...
    for key in keylist:
        ...
        if (key in list(kwargs.keys())):
            ...
            logger.error(
                " Invalid Input args %s is not in optional list %s" %
    ...
    for me_key in list(param_dict.keys()):
        ...
        if me_key in list(mandatory_options_list.keys()):
            if not set(mandatory_options_list[me_key]).issubset(
                    set(param_dict.keys())):
                logger.error(
                    " Invalid command invocation %s of mandatory list %s is not in arguments parovided" %
        ...
        if me_key in mutually_exclusive_keys:
            if me_key_found:
                logger.error(
                    " Invalid command invocation %s of mutual exclusive list %s " %
...
def osimages(*args, **kwargs):
    '''
    @summary:
        Use this function to retrieve information about, delete, and
        import OS images, OS-image profiles, device driver, and
        boot-options files.
        data_dictionary = osimages(input_args, key=values )

        Where KeyList is as follows

            keylist = [fileName, Id, profile,remoteFileServer,imageType,
            jobId, ...]
...
@param
    ...
    key    Returns the specified type of update. This can be one of the
           following values.
                all. Returns all information. This is the default value.
                currentVersion. Returns the current version of
                Lenovo XClarity Administrator.
                history. Returns the history of management-server updates.
                importDir. Returns the directory for the
                ...
                updatedDate. Returns the date when the last update was
                performed.

    action    The action to take. This can be one of the following values.
                apply   - install a management-server update.
                refresh - Retrieves information (metadata) about the
                latest available management-server updates from
                the Lenovo XClarity Support website.
                acquire - Downloads the specified management-server
                update packages from the Lenovo XClarity Support website.
                delete  - Use the DELETE method to delete update packages.
                - removeMetadata not supported
                import  - import fixids files

     fixids    comma separated fixids
     type      Type for Get with fixids
                changeHistory. Returns the change-history file for
                the specified management-server update.
                readme. Returns the readme file for the specified
...
@summary:
    Use this function to get and set complaince rules on
     Lenovo XClarity Administrator run this function as

    data_dictionary = managementserver( key1 = 'val1', key2 = 'val2', ...)

    Where KeyList is as follows

        keylist = ['con', 'id', 'name', 'targetResourceType',
        'targetGroup', 'content']
...
@summary:
    Use this function to get and set complaince
    rules on Lenovo XClarity Administrator
    run this function as
